...
public class GlobalExceptionHandler {
    ...
    // e.g. sending reportType: "MAGIC_ANALYSIS" when only EXECUTIVE, TECHNICAL, CUSTOM are valid
    ...
    public ResponseEntity<Map<String, String>> handleHttpMessageNotReadable(
        ...
        if (cause instanceof InvalidFormatException ife && ife.getTargetType() != null && ife.getTargetType().isEnum()) {
            ...
            String message = String.format(
                    ...
                    ife.getValue(),
                    ife.getPath().isEmpty() ? "unknown" : ife.getPath().get(ife.getPath().size() - 1).getPropertyName(),
                    validValues
            );
            return ResponseEntity.badRequest().body(Map.of("error", message));
        }
        return ResponseEntity.badRequest().body(Map.of("error", "Malformed JSON or unreadable request body."));
    }
